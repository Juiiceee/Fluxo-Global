...
const manageError = (error: any): string | undefined => {
	if ("cause" in error) {
		const { cause } = error;
		if ("shortMessage" in cause) {
...
function DetailModal({
	...
	const formatData = (obj: any) => {
		if (typeof obj !== "object" || obj === null) {
			return <div className="text-sm text-muted-foreground break-all">{String(obj)}</div>;
		}
									...
									<>
										...
										{hasArgs && (
											<div>
												...
												<div className="space-y-2">{formatData(args)}</div>
											</div>
										)}
									</>
								) : (
		...
		switch (state) {
			...
			case "completed":
				return {
					...
					cardBg:
						"bg-green-50 border-2 border-green-400 dark:bg-green-400/90 dark:border-green-500/40",
					titleColor: "text-green-900 dark:text-green-100",
					descriptionColor: "text-green-800 dark:text-white font-bold",
				};
			case "failed":
				return {
					icon: <XCircle className="w-5 h-5 text-red-600" />,
					title: "Failed",
					description: error ? manageError(error) : "Action failed. Please try again.",
					showButtons: false,
					showDetailsButton: false,
					iconBg: "bg-red-100 border-red-300 dark:bg-red-500/10 dark:border-red-500/20",
					cardBg: "bg-red-500 border-2 border-red-400 dark:bg-red-400/90 dark:border-red-500/40",
			...
			case "cancelled":
				return {
					...
					showDetailsButton: false,
					iconBg:
						"bg-gray-100 text-black border-gray-300 dark:bg-gray-500/10 dark:border-gray-500/20",
					cardBg:
						"bg-gray-50 text-black border-2 border-gray-400 dark:bg-gray-500/5 dark:border-gray-500/40",
				...
				<div className="flex items-center justify-between">
					<div className="flex items-center gap-3">
						...
						<div className="text-left">
							<div className={cn("font-semibold text-base", config.titleColor)}>{title}</div>
							<div className={cn("text-sm", config.descriptionColor)}>{config.description}</div>
						</div>
					</div>
